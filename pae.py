...
class PAE:
    ...
    def plummer(self, a, b):
        ...
        plummer = 1.0/tf.sqrt(dist + self.eps)
        return (plummer - tf.diag(tf.diag_part(plummer)))
    ...
    def create_model(self):
        ...
        # Plummer kernel
        self.true_intra_disc = self.plummer(self.true_input, self.true_input)
        self.true_inter_disc = self.plummer(self.true_input, self.z)
        self.fake_intra_disc = self.plummer(self.z, self.z)

        # Distribution matching
        n = self.batch_size*(self.batch_size-1)
        self.plummer = (tf.reduce_sum(self.fake_intra_disc) - 2.0*tf.reduce_sum(self.true_inter_disc) + tf.reduce_sum(self.true_intra_disc))/n
        ...
        # Total cost
        self.cost = self.reconstruction + self.LAMBDA*self.plummer
    ...
    def load_log(self,log):
        ...
        if os.path.exists(path):
            with open(path,'rb') as f:
                ...
                log['plummer_costs'] = data['plummer_costs']
    ...
    def train(self):
        ...
        log = {'costs':[],'rec_costs':[],'plummer_costs':[],'train_time':0}
        start = time.time()
        with tf.Session(config=config) as sess:        
            ...
            for t in t_obj:
                ...
                plummer_cost, _ = sess.run([self.plummer, self.cost_opt], feed_dict={
                    self.z: noise,
                    self.x: true_data,
                })
                ...
                check_nan = np.any(np.isnan(plummer_cost))
                assert not check_nan, 'plummer cost is nan'
                ...
                if self.global_step.eval() % 100 == 0: # saving at every iteration is not neccessary
                    ...
                    log['plummer_costs'].append(plummer_cost)
                ...
                if self.global_step.eval() % self.skip == 0:
                    ...
                    plt.plot(log['plummer_costs'])
                    plt.title('Plummer cost')
                    plt.xlabel('Iterations/100')
